...
local function packReady()
  ...
  state.packReady = ok and type(marker) == "string" and marker:match('"clip_count"%s*:%s*823') ~= nil
  state.packSource = state.packReady and "package" or nil
...
local function loadSource(index)
  ...
  local sound, err = cachedSoundData(bytes)
  if not sound then
    state.missing[index] = true
    return nil
  end
...
function Announcer.beginBattle(battle)
  ...
  if not packReady() then
    ...
    if not state.warnedMissingPack and logger and logger.info then
      ...
      logger:info("Stadium announcer voice pack is unavailable; battle audio unchanged")
    end
    return false
  end
  state.battle = battle
